...
typedef struct io_module_func {
	...
	int32_t   (*enqueue_pkts)(struct mtcp_thread_context *ctx, int nif);
	int32_t   (*pass_pkt)(struct mtcp_thread_context *ctx, int nif);
	int32_t   (*send_pkts)(struct mtcp_thread_context *ctx, int nif);
	int32_t   (*burst_pkts)(struct mtcp_thread_context *ctx, int nif);
