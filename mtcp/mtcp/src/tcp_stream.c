...
RaiseReadEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	if (stream->socket) {
		if (stream->socket->epoll & MTCP_EPOLLIN) {
			TRACE_EPOLL("mtcp %p Stream %p Socket %d add EPOLLIN\n", 
				mtcp, stream, stream->id); 
		...
		TRACE_EPOLL("mtcp %p Stream %p Socket %d: Raising read without a socket!\n", mtcp, stream, stream->id);
	}
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseWriteEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	if (stream->socket) {
		if (stream->socket->epoll & MTCP_EPOLLOUT) {
			...
			//fprintf(stdout, "Stream %d recv SYN_ACK packet!\n", stream->id);
#if BLOCKING_SUPPORT
		} else if (!(stream->socket->opts & MTCP_NONBLOCK)) {
		...
		TRACE_EPOLL("mtcp %p Stream %p Socket %d: Raising write without a socket!\n", mtcp, stream, stream->id);
	}
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseCloseEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	if (stream->socket) {
		...
		TRACE_EPOLL("mtcp %p Stream %p Socket %d: Raising close without a socket!\n", mtcp, stream, stream->id);
	}
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseErrorEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	if (stream->socket) {
		...
		TRACE_EPOLL("mtcp %p Stream %p Socket %d: Raising error without a socket!\n", mtcp, stream, stream->id);
	}
}
/*---------------------------------------------------------------------------*/
tcp_stream *
