		...
		uint32_t cur_ts, uint8_t *payload, uint32_t seq, int payloadlen)
{
	...
	if (TCP_SEQ_LT(seq + payloadlen, cur_stream->rcv_nxt)) {
		TRACE_ERROR("mtcp %p Stream %p Socket %d seq error!\n", 
						mtcp, cur_stream, cur_stream->id);
		return FALSE;
	}
	/* if payload exceeds receiving buffer, drop and send ack */
	if (TCP_SEQ_GT(seq + payloadlen, cur_stream->rcv_nxt + rcvvar->rcv_wnd)) {
		TRACE_ERROR("mtcp %p Stream %p Socket %d exceeds receiving buffer!\n", 
						mtcp, cur_stream, cur_stream->id);
		return FALSE;
	}
	...
	/*TRACE_EPOLL("mtcp %p Stream %p Socket %d data arrived. "
			"len: %d, ET: %u, IN: %u, OUT: %u\n", 
			mtcp, cur_stream, cur_stream->id, payloadlen, 
