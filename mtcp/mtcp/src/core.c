...
PrintThreadNetworkStats(mtcp_manager_t mtcp, struct net_stat *ns)
{
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		...
		ns->rx_drops[i] = mtcp->nstat.rx_drops[i] - mtcp->p_nstat.rx_drops[i];
		ns->tx_bytes[i] = mtcp->nstat.tx_bytes[i] - mtcp->p_nstat.tx_bytes[i];
		ns->write_called = mtcp->nstat.write_called;
		ns->payload_out = mtcp->nstat.payload_out;
#if NETSTAT_PERTHREAD
		if (CONFIG.eths[i].stat_print) {
			fprintf(stderr, "[CPU%2d] %s flows: %6u, "
					"RX: %7ld(pps) (drop_total::%5ld, drop:%5ld, err: %5ld), %5.2lf(Gbps), "
					"TX: %7ld(pps), %5.2lf(Gbps), "
					"write_call: %7ld, payload_out:%7ld\n", 
					mtcp->ctx->cpu, CONFIG.eths[i].dev_name, mtcp->flow_cnt, 
					ns->rx_packets[i], mtcp->nstat.rx_drops[i], ns->rx_drops[i], 
					ns->rx_errors[i], GBPS(ns->rx_bytes[i]), 
					ns->tx_packets[i], GBPS(ns->tx_bytes[i]), 
					mtcp->nstat.write_called, mtcp->nstat.payload_out
					);
		}
#endif
	}
...
{
	fprintf(stdout, "[CPU%2d] calls: %lu, waits: %lu, wakes: %lu, "
			"issued: %lu,\n" 
			"mtcpq_registered: %lu / mtcpq_handled: %lu\n"
			"shaw_registered: %lu / shaw_handled: %lu\n"
			"usrq_registered: %lu, usrq_copied: %lu/ usrq_handled: %lu\n"
			"shaw_invalidated: %lu,\n"
			"usrq_invalidated: %lu,\n",
			core, stat->calls, stat->waits, stat->wakes, 
			stat->issued, 
			stat->mtcpq_registered, stat->mtcpq_handled, 
			stat->shaw_registered, stat->shaw_handled, 
			stat->usrq_registered, stat->usrq_copied, stat->usrq_handled, 
			stat->shaw_invalidated, stat->usrq_invalidated);
	...
	for (i = 0; i < CONFIG.num_cores; i++) {
		if (running[i]) {
			...
			for (j = 0; j < CONFIG.eths_num; j++) {
				...
				g_nstat.write_called += ns.write_called;
				g_nstat.payload_out += ns.payload_out;
			}
#ifdef ENABLELRO
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		if (CONFIG.eths[i].stat_print) {
			fprintf(stderr, "[ ALL ] %s flows: %6u, "
					...
					"TX: %7ld(pps), %5.2lf(Gbps), "
					"write_call: %7ld, payload_out: %7ld, pkt_drop: %7ld\n", 
					CONFIG.eths[i].dev_name, gflow_cnt, 
					g_nstat.rx_packets[i], g_nstat.rx_errors[i], GBPS(g_nstat.rx_bytes[i]), 
					g_nstat.tx_packets[i], GBPS(g_nstat.tx_bytes[i]), 
					g_nstat.write_called, g_nstat.payload_out, mtcp->nstat.rx_drops[i]
					);
		}
	}
#ifdef ENABLELRO
...
FlushEpollEvents(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	if (EventQueLen(mtcpq) > 0) {
		/* while mtcp_queue have events */
		/* and usr_queue is not full */
		while ((mtcpq->end != mtcpq->start) && (!EventQueFull(usrq))) {
			/* copy the event from mtcp_queue to usr_queue */
			usrq->events[usrq->end] = mtcpq->events[mtcpq->start];
			rte_wmb();
			mtcpq->start = (mtcpq->start + 1) % mtcpq->size;
			usrq->end = (usrq->end + 1) % usrq->size;
			ep->stat.mtcpq_handled++;
			ep->stat.usrq_copied++;

			if (EventQueFull(usrq))
				continue;
			usrq->num_events = EventQueLen(usrq);

			/*if (mtcpq->start >= mtcpq->size)
				mtcpq->start = 0;*/
			mtcpq->num_events = EventQueLen(mtcpq);
		}
	}

	/* if there are pending events, wake up user */
	if (ep->waiting && ((ep->usr_queue->end != ep->usr_queue->start) || 
				(ep->usr_shadow_queue->end != ep->usr_shadow_queue->start))) {
