...
{
	return (eq->end - eq->start) & (eq->size - 1);
}
		...
		struct mtcp_epoll *ep, socket_map_t socket)
{
	...
	TRACE_EPOLL("Stream %p Socket %d at state %s\n", 
			stream, stream->id, TCPStateToString(stream));
	...
	if (socket->epoll & MTCP_EPOLLOUT) {
		...
		if (!sndvar->sndbuf || (sndvar->sndbuf && sndvar->snd_wnd > 0)) {
			if (!(socket->events & MTCP_EPOLLOUT)) {
				TRACE_EPOLL("Stream %p Socket %d: Adding write event\n", stream, socket->id);
		...
		int op, int sockid, struct mtcp_epoll_event *event)
{
	...
	if (op == MTCP_EPOLL_CTL_ADD) {
		...
		TRACE_EPOLL("[Core %d] Adding epoll Socket %d:(type %d) ET: %u, IN: %u, OUT: %u\n", 
				mctx->cpu, socket->id, socket->socktype, socket->epoll & MTCP_EPOLLET, 
		...
		struct mtcp_epoll_event *events, int maxevents, int timeout)
{
	...
	int spin = 0;
	while (ep->start == eq->end && spin < SPIN_THRESH) {
		spin++;
	}
...
wait:
	...
	while (eq->start == eq->end && eq_shadow->start == eq_shadow->end && timeout != 0) {
	...
	cnt = 0;
	//num_events = (eq->end - eq->start + eq->size) % eq->size;
	while ((eq->end != eq->start) && cnt < maxevents) {
		...
		if (validity) {
			...
			TRACE_EPOLL("[Core %d] Socket %d: Handled event %p. event: %s, "
					"start: %u, end: %u, num: %u\n", 
					mctx->cpu, event_socket->id, 
					&(eq->events[eq->start].ev),EventToString(eq->events[eq->start].ev.events), 
					eq->start, eq->end, eq->num_events);
			ep->stat.usrq_handled++;
		} else {
			TRACE_ERROR("[Core %d] Socket %d: event %s invalidated.\n", 
					mctx->cpu, eq->events[eq->start].sockid, 
					EventToString(eq->events[eq->start].ev.events));
			ep->stat.usrq_invalidated++;
		}
		event_socket->events &= (~eq->events[eq->start].ev.events);
		
		rte_wmb();
		
		eq->start = (eq->start + 1) % eq->size;
		
		eq->num_events = EventQueLen(eq);
		if (EventQueFull(eq)) {
			eq->start = eq->end;
		}
	}
	...
	while ( eq->end != eq->start && cnt < maxevents) {
		...
		if (validity) {
			...
			TRACE_EPOLL("[Core %d] Socket %d: eq %p Handled event %p. event: %s, "
					"start: %u, end: %u, num: %u\n", 
					mctx->cpu, event_socket->id, eq, 
					&(eq->events[eq->start].ev),
					EventToString(eq->events[eq->start].ev.events), 
					eq->start, eq->end, eq->num_events);
			ep->stat.shaw_handled++;
		} else {
			TRACE_ERROR("[Core %d] Socket %d: event %s invalidated.\n", 
					mctx->cpu, eq->events[eq->start].sockid, 
					EventToString(eq->events[eq->start].ev.events));
			ep->stat.shaw_invalidated++;
		}
		event_socket->events &= (~eq->events[eq->start].ev.events);
		...
		int queue_type, socket_map_t socket, uint32_t event)
{
	...
	if (!ep || !socket || !event){
		TRACE_ERROR("Non ep or socket or event Socket %d event state!\n", socket->id);
		return -1;
	}
	...
	if (socket->events & event) {
		TRACE_ERROR("Non-alive Socket %d event state!\n", socket->id);
		return 0;
	}
	...
	eq->end = (eq->end + 1) % eq->size;

	eq->num_events = EventQueLen(eq);

#if 0
	fprintf(stdout, "Que %d Stream %p Socket %d New event %p : %s, start: %u, end: %u, num: %u\n",
			queue_type, socket->stream, eq->events[index].sockid, 
			eq->events + index,
	...
	if (queue_type == USR_EVENT_QUEUE){
		ep->stat.usrq_registered++;
		pthread_mutex_unlock(&ep->epoll_lock);
	}else if (queue_type == MTCP_EVENT_QUEUE){
		ep->stat.mtcpq_registered++;
	} else if (queue_type == USR_SHADOW_EVENT_QUEUE) {
		ep->stat.shaw_registered++;
	}
