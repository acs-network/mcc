...
logger wget_logger("wget");
/*----------------------------------------------------------------------------*/
vector<thread> app_thread;
...
//			"Connection: Keep-Alive\r\n\r\n", 
	...
	ctx->stat.writes += wr;
	wget_logger.trace("Socket {} HTTP Request of {} bytes. sent.", sockid, wr);
...
DownloadComplete(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
	...
	wget_logger.trace("Socket {} File download complete!", sockid);
	gettimeofday(&wv->t_end, NULL);
    wv->recv_ts = system_clock::now();
    auto delay = duration_cast<microseconds>(wv->recv_ts - wv->send_ts).count();
	...
	wget_logger.trace("Socket {} Total received bytes: {} ({}MB)", 
			sockid, wv->recv, wv->recv / 1000000);
	wget_logger.trace("Socket {} Total spent time: {} us", sockid, delay);
	if (tdiff > 0) {
		wget_logger.trace("Socket {} Average bandwidth: {}[MB/s]", 
				sockid, (double)wv->recv / tdiff);
	}
	ctx->stat.sum_resp_time += delay;
	if (tdiff > ctx->stat.max_resp_time)
		ctx->stat.max_resp_time = delay;
...
RunWgetMain(int core)
{
	...
	while (!done[core]) {
		...
		nevents = mtcp_epoll_wait(mctx, ep, events, maxevents, 0);
		ctx->stat.waits++;
...
main(int argc, char **argv)
{
	...
	for (i = ((process_cpu == -1) ? 0 : process_cpu); i < core_limit; i++) {
        ...
        thread t(RunWgetMain, cores[i]);
        app_thread.push_back(std::move(t));
